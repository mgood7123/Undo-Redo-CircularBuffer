 ...
 */
jbyteArray JniHelpers::Strings::newByteArray(JNIEnv * env, std::string string) {
    int byteCount = string.length();
    const jbyte* pNativeString = reinterpret_cast<const jbyte*>(string.c_str());
 ...
 * @return a std::string converted to a jbyteArray
 */
jstring JniHelpers::Strings::newString(JNIEnv * env, std::string string) {
    return newString(env, newByteArray(env, string));
};
...
jstring JniHelpers::Strings::newString(JNIEnv * env, jbyteArray byteArray) {
    ...
    jstring jMessage = reinterpret_cast<jstring>(
            env->NewObject(stringClass, charsetConstructor, byteArray, charset));
    return jMessage;
};
